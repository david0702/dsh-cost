...
// 峰谷价自北京时间 2026-08-17 00:00（UTC 2026-08-16 16:00）起生效
const PEAK_EPOCH_UTC = Date.UTC(2026, 7, 16, 16, 0, 0);
const MODE_META = {
  flat: { label: "8-17 前" },
  peak: { label: "高峰（工作日）" },
  offpeak: { label: "空闲" },
...
function modeAt(ms) {
  if (ms < PEAK_EPOCH_UTC) return "flat";
...
function apply(ctx, config) {
  ...
  const notifierPath = (config && config.notifierPath) || "";
  ...
  ctx.effect(() => webServer.register({
    ...
    handler: async (req, res) => {
      ...
      try {
        const url = new URL(req.url || "/", "http://x");
        const msg = url.searchParams.get("msg") || "DSH 费用提醒";
        if (!notifierPath) return send(200, { ok: false, error: "notifier not configured" });
        await new Promise((resolve, reject) => {
          const child = spawn(notifierPath, [msg], { windowsHide: true, stdio: ["ignore", "pipe", "pipe"] });
          child.stderr.on("data", (d) => { void d; });
          child.on("error", reject);
          child.on("close", (code) => { if (code === 0) resolve(); else reject(new Error("exit " + code)); });
        });
        send(200, { ok: true });
      } catch (err) {
        send(200, { ok: false, error: String((err && err.message) || err) });
      }
    },
  }), "dsh-cost: notify route");
}
