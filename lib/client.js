window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function CostCard(props) {
      ...
      var order = ["peak", "offpeak", "flat"];
      var meta = {
        flat: { label: "历史 8-17 前", color: "var(--dsw-static-neutral-bluish-400)", muted: true },
        peak: { label: "高峰", color: "var(--dsw-alias-state-warn-primary)" },
        offpeak: { label: "空闲", color: "var(--dsw-alias-state-success-primary)" }
      };
      var rows2 = [];
      ...
      var barOrder = ["flat", "peak", "offpeak"];
      var barSegs = [];
    ...
    function CostLine(props) {
      ...
      // 费用阈值告警：本会话累计费用超过阈值时弹一次通知（可在下面改阈值）
      var alertedRef = react.useRef(false);
      var COST_ALERT_THRESHOLD = 50;
      react.useEffect(function () {
        if (!accurate || accurate.cost == null) return;
        if (accurate.cost >= COST_ALERT_THRESHOLD && !alertedRef.current) {
          alertedRef.current = true;
          var msg = "DSH 费用提醒：本会话累计 ¥" + formatCost(accurate.cost);
          window.fetch("/api/dsh-cost/notify?msg=" + encodeURIComponent(msg), { cache: "no-store" }).catch(function () {});
        }
      }, [accurate]);
